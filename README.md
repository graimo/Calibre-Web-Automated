...
- **Opt-in toggle** *"Per-user book ownership"* in **CWA Settings** (off by default). When on, **every** user — admins included — sees only the books they own. Admins are isolated by default too, but get a **"Books view" toggle** in the top bar to reveal the whole library on demand (per session), so they keep full oversight/management without seeing everyone's books all the time. Turning the setting off restores the shared-library behaviour.
